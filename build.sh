...
iso_name=cluster-deploy
iso_label="DEPLOY_$(date +%Y%m)"
iso_publisher="Contoso Ltd <http://www.contoso.com>"
iso_application="Cluster Deployment Toolkit CD"
...
make_syslinux() {
    ...
    for _cfg in ${script_path}/syslinux/*.cfg; do
        sed "s|%APPLICATION%|${iso_application}|g;
             s|%NAME%|${iso_name}|g;
...
make_prune_pkgs() {
    local wanted=( $(xargs < packages.x86_64) )
    cat > ${work_dir}/x86_64/airootfs/root/remove_pkgs.sh << EOF
#!/usr/bin/env bash
all=( \$(pacman -Qq) )
for i in ${wanted[@]}; do
    all=( \${all[@]//*\${i}*} )
...
yes | pacman -Rsu \${all[@]}
EOF
    chmod a+x ${work_dir}/x86_64/airootfs/root/remove_pkgs.sh
    mkarchiso ${verbose} -w "${work_dir}/x86_64" -C "${work_dir}/pacman.conf" -D "${install_dir}" -r '/root/remove_pkgs.sh' run
    rm ${work_dir}/x86_64/airootfs/root/remove_pkgs.sh
}
